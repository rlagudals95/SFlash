...
const GET_MINE_AND_MYLIKE_IN_MAP = "GET_MINE_AND_MYLIKE_IN_MAP";
const RESET_CATEGORY_IN_MAP = "RESET_CATEGORY_IN_MAP";
const RESET_MINE_AND_MYLIKE_IN_MAP = "RESET_MINE_AND_MYLIKE_IN_MAP";
...
const getMineAndMyLikeInMap = createAction(GET_MINE_AND_MYLIKE_IN_MAP, (mine_and_mylike_in_map) => ({
  mine_and_mylike_in_map,
}))

const resetCategoryInMap = createAction(RESET_CATEGORY_IN_MAP, (is_category_in_map) => ({
  is_category_in_map,
}));
const resetMineAndMyLikeInMap = createAction(RESET_MINE_AND_MYLIKE_IN_MAP, (is_mine_and_mylike_in_map) => ({ 
  is_mine_and_mylike_in_map,
}));
...
const initialState = {
  ...
  is_mine_and_mylike_in_map: [], // 내가 작성한 게시물, 내가 좋아요한 게시물 카테고리를 넣어준다.
...
export default handleActions(
  {
    ...
    [GET_MINE_AND_MYLIKE_IN_MAP]: (state, action) =>
      produce(state, (draft) => {
        // 카테고리 상태에 카테고리 명을 넣어준다
        draft.is_mine_and_mylike_in_map.push(action.payload.mine_and_mylike_in_map);

        // 다른 카테고리가 들어오면 붙여주고
        draft.is_mine_and_mylike_in_map = draft.is_mine_and_mylike_in_map.reduce((acc, cur) => {
          if (acc.findIndex((a) => a === cur) === -1) {
            return [...acc, cur];
          } else {
            // 현재 카테고리상태 배열을 모두 for문으로 돌린다
    ...
    [RESET_MINE_AND_MYLIKE_IN_MAP]: (state, action) =>
      produce(state, (draft) => {
        draft.is_mine_and_mylike_in_map = []; // 전체보기 효과를 위해 이렇게 리셋 해준다
      }),
...
const actionCreators = { 
  getCategoryInMap,
  getMineAndMyLikeInMap,
  resetCategoryInMap,
  resetMineAndMyLikeInMap,
  selectCategoryInMap 
};
